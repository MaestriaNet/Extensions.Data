...
- Create file with extensions method name in folder `src` and unit test in `tests/`.
- Extensions method should receive `this` argument.
...
2. **Publish Preview** – Triggered when a PR is merged into `main`. It determines a preview version (e.g., `2.1.2-preview.pr-61`), builds the package and publishes it to the [MyGet](https://www.myget.org/feed/Packages/maestrianet) feed for downstream testing.
